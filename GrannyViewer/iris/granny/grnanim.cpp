...
void BoneAnim::load(cGrannyStream * file,dword offset,dword baseOffset,dword peers)
{
	assert(file);
  union
  {
	...
	for (i=0;i<numTranslates;i++)
	{
		...
		pDebug.Log(LEVEL_INFO1,"translate timeline anim id; %2d, (%2d, %f)", id, translateTimeline.size()-1, fd.f);
	};
	length=fd.f;
	for (i=0;i<numQuaternions;i++)
	{
		...
		pDebug.Log(LEVEL_INFO1,"quaternion timeline anim id; %2d, (%2d, %f)", id, quaternionTimeline.size()-1, fd.f);
	};
	...
	pDebug.Log(LEVEL_INFO1,"bone anim - id:% 2d translation count: %2d time: %2d quaternion count: %2d time: %2d unknown:%2d length:%f", 
		id,translates.size(),translateTimeline.size(),quaternions.size(),quaternionTimeline.size(),numUnknowns, length);
}
...
void Animations::load(cGrannyStream * file,dword aOffset,dword baseOffset,dword peers)
{
	assert(file);
	BoneAnim *bone;
	for (unsigned int i=0;i<peers;)
	{
		...
		default:
			{
				pDebug.Log(LEVEL_ERROR, "Unknown Animations Chunk: 0x%x", chunk);
				//assert(!"Unknown Animations Chunk");
				return;
			}
		}
		i+=children+1;
