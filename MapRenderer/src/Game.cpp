...
void Game::LoadMuls(std::string mulpath)
{
    ...
    int mapID;
    if (nConfig::uomap_nr == UOMAP_MAP1) {
     mapID=0;
    }
    else {
     mapID=nConfig::uomap_nr;
    }

    char mapstr[2];
    mapstr[1] = 0;
    sprintf(mapstr, "%i", (int) mapID);
    string mul_map0 = mulpath+"map" + string(mapstr) + ".mul";
    string mul_statics0 = mulpath+"statics" + string(mapstr) + ".mul";
    string mul_staidx0 = mulpath+"staidx" + string(mapstr) + ".mul";

    std::cout << "MAP: " << mul_map0 <<std::endl;

    string mul_texmaps = mulpath+"texmaps.mul";
    string mul_texidx = mulpath+"texidx.mul";

    pDebug.Log("SYS | Loading UO Files....");	
    pMapLoader = new UOMapLoader((char*)mul_map0.c_str(), (char*)mul_statics0.c_str(), (char*)mul_staidx0.c_str(), mapID);
