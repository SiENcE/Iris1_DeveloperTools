...
type
   ...
   TTextureManager = class (TObject)
       ...
       public
         ...
         procedure RemoveTexture (AMD5Sum: String);

         procedure ResetUseCount;
...
procedure TTextureManager.ResetUseCount;
var
    Index: Integer;
begin
    For Index := 0 to Count - 1 do
        Textures[Index].FUseCount := 0;
end;

procedure TTextureManager.RemoveTexture (AMD5Sum: String);
var
    ATexture: TJPEGTexture;
    Index: Integer;
begin
    ATexture := GetTexture (AMD5Sum);
    if Assigned (ATexture) then
        if ATexture.UseCount = 0 then begin
            FMap.remove([ATexture.MD5Sum]);
            For Index := FTextures.Count - 1 downto 0 do
                if FTextures[Index] = ATexture  then
