...
type
  ...
  TEditForm = class(TForm)
    ...
    CoordSystem1: TMenuItem;
    procedure CoordSystem1Click(Sender: TObject);
...
begin
    ...
    FEngine.MakeCurrent (GetDC (Handle));
    ...
    if (PageControl.ActivePage = LightingTabSheet) and (FModel.LightSourceOptions.FModelIsLightSource) then begin
        glPushMatrix ();
        glTranslatef (-0.5, -0.5, -FModel.Height / 20);
        glDisable (GL_LIGHTING);
        SetColor (Shape1.Brush.Color);
        glBegin(GL_LINES);
        with FModel.LightSourceOptions do begin
...
{          glVertex3f(FX, FY - FRadius, FZ);
    ...
    if (PageControl.ActivePage = ParticleTabSheet) and (mfModelHasParticleEffect in FModel.Flags) then begin
        glPushMatrix ();
        glTranslatef (-0.5, -0.5, -FModel.Height / 20);
        glTranslatef (FModel.EffectOptions.FX, FModel.EffectOptions.FY, FModel.EffectOptions.FZ);
        if FModel.EffectOptions.FRotateEffect then begin
            glRotatef (FModel.EffectOptions.FAngle1, 1, 0, 0);
            glRotatef (FModel.EffectOptions.FAngle2, 0, 1, 0);
        end;
        glDisable (GL_LIGHTING);
        SetColor (clGreen);
        glBegin(GL_LINES);
    ...
    if Render_CoordSystem then begin
        glDisable (GL_LIGHTING);
        glBegin(GL_LINES);
        SetColor (clRed);
        glVertex3f(0, 0, 0);
...
var
    LightSourceOptions: TLightSourceOptions;
begin
   ColorDialog.Color := Shape1.Brush.Color;
   if Assigned(FModel) and ColorDialog.Execute then begin
      LightSourceOptions := FModel.LightSourceOptions;
      LightSourceOptions.FR := ColorDialog.Color and 255;
      LightSourceOptions.FG := (ColorDialog.Color shr 8) and 255;
      LightSourceOptions.FB := (ColorDialog.Color shr 16) and 255;
      FModel.LightSourceOptions := LightSourceOptions;
...
begin
   if Assigned (FModel) then begin
      LightSourceOptions := FModel.LightSourceOptions;
      LightSourceOptions.FModelIsLightSource := Checkbox2.Checked;
      FModel.LightSourceOptions := LightSourceOptions;
   end;
   Render;
end;
...
begin
   if Assigned (FModel) then begin
      LightSourceOptions := FModel.LightSourceOptions;
      LightSourceOptions.FFlickering := Checkbox3.Checked;
      FModel.LightSourceOptions := LightSourceOptions;
   end;
   Render;
end;
...
var
    LightSourceOptions: TLightSourceOptions;
begin
   if Assigned (FModel) and (Edit14.Text <> '') then begin
      LightSourceOptions := FModel.LightSourceOptions;
      LightSourceOptions.FX := StrToFloat(Edit14.Text);
      FModel.LightSourceOptions := LightSourceOptions;
   end;
   Render;
end;
...
var
    LightSourceOptions: TLightSourceOptions;
begin
   if Assigned (FModel) and (Edit15.Text <> '') then begin
      LightSourceOptions := FModel.LightSourceOptions;
      LightSourceOptions.FY := StrToFloat(Edit15.Text);
      FModel.LightSourceOptions := LightSourceOptions;
   end;
   Render;
end;
...
var
    LightSourceOptions: TLightSourceOptions;
begin
   if Assigned (FModel) and (Edit16.Text <> '') then begin
      LightSourceOptions := FModel.LightSourceOptions;
      LightSourceOptions.FZ := StrToFloat(Edit16.Text);
      FModel.LightSourceOptions := LightSourceOptions;
   end;
   Render;
end;
...
begin
   if Assigned (FModel) and (Edit17.Text <> '') then begin
      LightSourceOptions := FModel.LightSourceOptions;
      LightSourceOptions.FRadius := StrToFloat(Edit17.Text);
      if LightSourceOptions.FRadius < 0.5 then
          LightSourceOptions.FRadius := 0.5;
      FModel.LightSourceOptions := LightSourceOptions;
   end;
   Render;
end;
...
begin
   if Assigned (FModel) and (Edit18.Text <> '') then begin
      LightSourceOptions := FModel.LightSourceOptions;
      LightSourceOptions.FFlickerMinDelay := StrToInt(Edit18.Text);
      if LightSourceOptions.FFlickerMinDelay < 100 then
          LightSourceOptions.FFlickerMinDelay := 100;
      FModel.LightSourceOptions := LightSourceOptions;
   end;
   Render;
end;
...
begin
   if Assigned (FModel) and (Edit19.Text <> '') then begin
      LightSourceOptions := FModel.LightSourceOptions;
      LightSourceOptions.FFlickerMaxDelay := StrToInt(Edit19.Text);
      if LightSourceOptions.FFlickerMaxDelay < LightSourceOptions.FFlickerMinDelay + 10 then
          LightSourceOptions.FFlickerMaxDelay := LightSourceOptions.FFlickerMinDelay + 10;
      FModel.LightSourceOptions := LightSourceOptions;
   end;
   Render;
end;
...
begin
   if Assigned (FModel) and (Edit20.Text <> '') then begin
      LightSourceOptions := FModel.LightSourceOptions;
      LightSourceOptions.FFlickerAmount := StrToInt(Edit20.Text);
      if LightSourceOptions.FFlickerAmount < 1 then
          LightSourceOptions.FFlickerAmount := 1;
      if LightSourceOptions.FFlickerAmount > 100 then
          LightSourceOptions.FFlickerAmount := 100;
      FModel.LightSourceOptions := LightSourceOptions;
   end;
   Render;
end;
...
begin
  ...
  LightSourceOptions := FModel.LightSourceOptions;
  LightSourceOptions.FX := LightSourceOptions.FX - Vector[0];
  LightSourceOptions.FY := LightSourceOptions.FY - Vector[1];
  LightSourceOptions.FZ := LightSourceOptions.FZ - Vector[2];
  FModel.LightSourceOptions := LightSourceOptions;

  Edit14.Text := Format('%.2f', [ FModel.LightSourceOptions.FX ]);
  Edit15.Text := Format('%.2f', [ FModel.LightSourceOptions.FY ]);
  Edit16.Text := Format('%.2f', [ FModel.LightSourceOptions.FZ ]);
end;
...
begin
     TextureManagerForm.IsModal := True;
...
begin
   ...
   pLightClipboard := FModel.LightSourceOptions;
end;
...
begin
   if not pLightClipboardEmpty then
     if MessageDlg ('Are you sure to overwrite the current light settings?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then begin
         FModel.LightSourceOptions := pLightClipboard;
         ShowLightSettings;
     end;
end;
...
begin
   ...
   Shape1.Brush.Color := FModel.LightSourceOptions.FR + FModel.LightSourceOptions.FG shl 8 + FModel.LightSourceOptions.FB shl 16;
   Checkbox2.Checked := FModel.LightSourceOptions.FModelIsLightSource;
   Checkbox3.Checked := FModel.LightSourceOptions.FFlickering;
   Edit14.Text := Format('%.2f', [ FModel.LightSourceOptions.FX ]);
   Edit15.Text := Format('%.2f', [ FModel.LightSourceOptions.FY ]);
   Edit16.Text := Format('%.2f', [ FModel.LightSourceOptions.FZ ]);
   Edit17.Text := Format('%.2f', [ FModel.LightSourceOptions.FRadius ]);
   Edit18.Text := IntToStr( FModel.LightSourceOptions.FFlickerMinDelay );
   Edit19.Text := IntToStr( FModel.LightSourceOptions.FFlickerMaxDelay );
   Edit20.Text := IntToStr( FModel.LightSourceOptions.FFlickerAmount );
...
begin
   ...
   Checkbox7.Checked := FModel.EffectOptions.FRotateEffect;

   Edit21.Text := Format('%.2f', [ FModel.EffectOptions.FX ]);
   Edit22.Text := Format('%.2f', [ FModel.EffectOptions.FY ]);
   Edit23.Text := Format('%.2f', [ FModel.EffectOptions.FZ ]);
   Edit25.Text := Format('%.2f', [ FModel.EffectOptions.FAngle1 ]);
   Edit26.Text := Format('%.2f', [ FModel.EffectOptions.FAngle2 ]);
   Trackbar1.Position := FModel.ShadowIntensity;
...
begin
    ...
    try
        if Form.ShowModal = mrOK then begin
            ...
            AModel := TModel.CreateFrom3DS(0, Form.FileName, Form.ScaleMode, Form.ScaleFactor, Form.TranslateToDefaultPos);
            try
...
var
  EffectOptions: TParticleEffectOptions;
begin
  EffectOptions := FModel.EffectOptions;
  EffectOptions.FRotateEffect := Checkbox7.Checked;
  FModel.EffectOptions := EffectOptions;
  Render;
end;
...
var
  EffectOptions: TParticleEffectOptions;
begin
   if Assigned (FModel) and (Edit21.Text <> '') then begin
      EffectOptions := FModel.EffectOptions;
      EffectOptions.FX := StrToFloat(Edit21.Text);
      FModel.EffectOptions := EffectOptions;
   end;
   Render;
end;
...
var
  EffectOptions: TParticleEffectOptions;
begin
   if Assigned (FModel) and (Edit22.Text <> '') then begin
      EffectOptions := FModel.EffectOptions;
      EffectOptions.FY := StrToFloat(Edit22.Text);
      FModel.EffectOptions := EffectOptions;
   end;
   Render;
end;
...
var
  EffectOptions: TParticleEffectOptions;
begin
   if Assigned (FModel) and (Edit23.Text <> '') then begin
      EffectOptions := FModel.EffectOptions;
      EffectOptions.FZ := StrToFloat(Edit23.Text);
      FModel.EffectOptions := EffectOptions;
   end;
   Render;
end;
...
var
  EffectOptions: TParticleEffectOptions;
begin
   if Assigned (FModel) and (Edit25.Text <> '') then begin
      EffectOptions := FModel.EffectOptions;
      EffectOptions.FAngle1 := StrToFloat(Edit25.Text);
      FModel.EffectOptions := EffectOptions;
   end;
   Render;
end;
...
var
  EffectOptions: TParticleEffectOptions;
begin
   if Assigned (FModel) and (Edit26.Text <> '') then begin
      EffectOptions := FModel.EffectOptions;
      EffectOptions.FAngle2 := StrToFloat(Edit26.Text);
      FModel.EffectOptions := EffectOptions;
   end;
   Render;
end;
...
begin
  ...
  EffectOptions := FModel.EffectOptions;
  EffectOptions.FX := EffectOptions.FX - Vector[0];
  EffectOptions.FY := EffectOptions.FY - Vector[1];
  EffectOptions.FZ := EffectOptions.FZ - Vector[2];
  FModel.EffectOptions := EffectOptions;

  Edit21.Text := Format('%.2f', [ EffectOptions.FX ]);
  Edit22.Text := Format('%.2f', [ EffectOptions.FY ]);
  Edit23.Text := Format('%.2f', [ EffectOptions.FZ ]);
end;

procedure TEditForm.AligntoGrid1Click(Sender: TObject);
var
  S: String;
  GridSize: Single;
begin
    S := InputBox ('Align to Grid', 'Gridsize', '');
    if S <> '' then begin
        GridSize := StrToFloat (S);
        FModel.AlignToGrid (GridSize);
        Render;
    end;
end;

procedure TEditForm.CoordSystem1Click(Sender: TObject);
begin
  CoordSystem1.Checked := not CoordSystem1.Checked;
  Render_CoordSystem := CoordSystem1.Checked;
  Render;
end;
