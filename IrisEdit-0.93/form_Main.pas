...
uses
  ...
  Unit_OpenGL, form_Progress, Unit_TextureManager, ExtDlgs;
      ...
      For Index := 0 to FItems.Count - 1 do begin
          ...
          if ItemEntry.Category = Category then begin
               ...
               if Assigned(ItemEntry.Model) then begin
                  ...
                  S := '';
                  if mfModelisTileable in ItemEntry.Model.Flags then  S := S + 'T';
                  if mfModelCastsShadows in ItemEntry.Model.Flags then  S := S + 'S';
                  if mfModelHasParticleEffect in ItemEntry.Model.Flags then  S := S + 'P';
                  if ItemEntry.Model.FLightSourceOptions.FModelIsLightSource then
                        S := S + 'L';
